...
// Define local interfaces for component-specific types
interface CriticalRole {
  id: string;
  title: string;
  currentHolder: EmployeeData & {
    tenure: number;
    readiness: string;
    criticality: string;
    image?: string;
  };
  potentialSuccessors: Array<SuccessorCandidate & {
    title?: string;
    readiness?: string;
    match?: number;
    image?: string;
...
// Remove duplicate interface since it's imported from types/employee.ts

export default function SuccessionPlanningPage() {
  const [searchQuery, setSearchQuery] = useState('');
  const [searchResults, setSearchResults] = useState<SearchResultsDataType | null>(null);
  ...
  // Define fetchHighRiskEmployees with useCallback to prevent recreation on every render
  const fetchHighRiskEmployees = React.useCallback(async () => {
    ...
    try {
      ...
      console.log("At-risk employees data:", data);
  ...
  // fetchHighRiskEmployees is defined above with useCallback
                ...
                <div>
                  <Card className="mb-6 hover:shadow-md transition-shadow">
                    ...
                    <CardContent>
                      ...
                      {/* Attrition Risk Details component - Temporarily disabled
                      {insightMode && (
                        <div className="mb-4 p-4 bg-yellow-50 border border-yellow-200 rounded-md">
                          <h4 className="font-medium text-yellow-800 mb-2">Attrition Risk Analysis</h4>
                          <p className="text-sm text-yellow-700">Risk Level: {searchResults.employee.attritionRisk}</p>
                          <p className="text-sm text-yellow-700 mt-1">{searchResults.employee.primaryExplanation}</p>
                        </div>
                      )}
                      */}
