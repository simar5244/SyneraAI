...
// Extend d3.selection prototype to safely handle null values
const originalProperty = d3.selection.prototype.property;
d3.selection.prototype.property = function() {
  if (!this.node()) return this; // If node is null, return the selection without changes
  return originalProperty.apply(this, arguments);
};
...
export default function OrgChart({
  ...
  useEffect(() => {
    ...
    if (!svg.node()) return; // Ensure the node exists
    ...
    const zoomBehavior = d3.zoom()
      ...
      .on('zoom', (event) => {
        ...
        if (g.node()) { // Check if g exists before applying transform
    ...
    return () => {
      try {
        ...
        console.error('Error removing zoom behavior:', error);
      }
    };
  }, [dimensions]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      nodeElements
        ...
        .on('mouseout', function(event, d: OrgNode) {
          d3.select(this)
      ...
      if (!readOnly) {
        try {
          ...
          console.error('Error setting up background click handler:', error);
        }
      }
  ...
  }, [nodes, connections, dimensions, zoom, readOnly, connectionMode, sourceNode, isCreatingNode, onNodeUpdate, onNodeDelete, onConnectionAdd, theme, handleNodeClick]);
  ...
  const handleNodeClick = useCallback((node: OrgNode) => {
    ...
    setNodeDialogOpen(true);
  }, [readOnly, connectionMode, sourceNode, onConnectionAdd]);
  ...
  return (
    <div ref={wrapperRef} style={{ width: '100%', height: '100%', position: 'relative' }}>
      ...
      {!readOnly && (
        ...
        >
          ...
          {connectionMode && sourceNode && (
            <Box marginTop={2} padding={1} bgcolor="#b3e5fc" color="#01579b" borderRadius={1}>
      ...
      <Dialog open={nodeDialogOpen} onClose={() => setNodeDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Box component="form" style={{ marginTop: 16 }} noValidate autoComplete="off">
            <TextField
              label="Name"
              fullWidth
              margin="normal"
            ...
            <Typography id="skill-level-slider" gutterBottom style={{ marginTop: 16 }}>
            ...
            <Typography id="workload-capacity-slider" gutterBottom style={{ marginTop: 16 }}>
            ...
            <Box style={{ marginTop: 24, display: 'flex', justifyContent: 'flex-end' }}>
              <Button onClick={() => setNodeDialogOpen(false)} style={{ marginRight: 8 }}>
                Cancel
              </Button>
              <Button 
                variant="contained" 
