...
export default function RequestPasswordResetPage() {
  ...
  // Add new useEffect to clear verification message when on complete step
  useEffect(() => {
  ...
  const handleRequestSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError(null);
    // Custom email validation to avoid browser native pattern errors
    ...
    try {
      ...
      // Don't redirect, just move to verification step on the same page
    } catch (error: any) {
  ...
  const handleVerifySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Clear verification success message and move to completion step
      setSuccessMessage(null);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative">
      ...
      />
      <div
        className="flex w-[900px] h-[640px] overflow-hidden rounded-xl shadow-2xl bg-white relative z-10"
        style={{ width: '800px !important', height: '550px !important' }}
      >
        {/* Left side - Reset form */}
        <div
          className="p-8 bg-white overflow-y-auto"
          style={{ width: '350px !important', height: '100%', flexShrink: 0 }}
        >
          ...
          {currentStep === 'request' && (
            <form noValidate onSubmit={handleRequestSubmit} className="space-y-2.5">
              <div>
          ...
          {currentStep === 'verify' && (
            <form noValidate onSubmit={handleVerifySubmit} className="space-y-2.5">
              <div>
                <label htmlFor="email-readonly" className="block text-sm font-medium text-gray-600 mb-0.5">
                  Email Address
                </label>
                <input
                  id="email-readonly"
                  ...
                  value={email}
                  readOnly
                  className="mt-0 block w-full px-2.5 py-1 bg-gray-50 border border-gray-300 rounded-md shadow-sm text-sm text-gray-900"
                />
              </div>
        ...
        <div
          className="bg-gradient-to-br from-purple-600 to-pink-500 p-8 flex flex-col justify-center items-center text-white relative overflow-hidden"
          style={{ width: '528px !important', height: '100%', flexShrink: 0 }}
        >
          <img
