...
try:
    from attrition_score import call_gemini_llm  # LLM fallback for task intensity
except ImportError:
    logging.warning("attrition_score.call_gemini_llm not found, using stub")
    def call_gemini_llm(*args, **kwargs):
        return {"matched_task": "", "similarity": 0.0, "intensity_score": 0.5, "explanation": "LLM fallback unavailable", "analysis_method": "stub"}
...
def get_representative_benchmark_tasks(n: int) -> List[Dict[str, Any]]:
    """
    Return top-n benchmark tasks as examples for LLM fallback.
    """
...
def get_task_intensity(duty_description: str) -> Dict[str, Any]:
    ...
    if not is_compound:
        ...
        if not top_matches:
            # No matches at all - use LLM fallback if available
            llm_result = call_gemini_llm(
                duty_description,
                "task_intensity",  # specify task type for LLM
                get_representative_benchmark_tasks(15)
            )
            
            return {
                "original_task": duty_description,
                "matched_task": llm_result.get("matched_task", "no matches found"),
                "similarity": llm_result.get("similarity", 0.0),
                "intensity_score": llm_result.get("intensity_score", 0.5),
                "explanation": llm_result.get("explanation", "No good matches found"),
                "analysis_method": llm_result.get("analysis_method", "llm_fallback")
            }
        ...
        # For lower similarity (below 0.55), use LLM fallback
        llm_result = call_gemini_llm(
            duty_description,
            "task_intensity",  # specify task type for LLM
            get_representative_benchmark_tasks(15)
        ...
        return {
            "original_task": duty_description,
            "matched_task": llm_result.get("matched_task", "low similarity matches"),
            "similarity": llm_result.get("similarity", best_match["similarity"]),
            "intensity_score": llm_result.get("intensity_score", 0.5),
            "top_matches": top_matches[:2],  # Include top 2 matches for reference
            "explanation": llm_result.get("explanation", "Low similarity to benchmark tasks"),
            "analysis_method": llm_result.get("analysis_method", "llm_fallback_low_similarity")
        }
    ...
    # For compound tasks, split and analyze each component
        ...
        if not top_matches:
            # No matches at all - use LLM fallback
            llm_result = call_gemini_llm(
                duty_description,
                "task_intensity",  # specify task type for LLM
                get_representative_benchmark_tasks(15)
        ...
        # For low similarity, use LLM fallback
        llm_result = call_gemini_llm(
            duty_description,
            "task_intensity",  # specify task type for LLM
            get_representative_benchmark_tasks(15)
        ...
        return {
            "original_task": duty_description,
            "matched_task": llm_result.get("matched_task", "low similarity compound"),
            "similarity": llm_result.get("similarity", best_match["similarity"]),
            "intensity_score": llm_result.get("intensity_score", 0.5),
            "top_matches": top_matches[:2],  # Include top 2 matches for reference
            "explanation": llm_result.get("explanation", "Low similarity compound task"),
            "analysis_method": llm_result.get("analysis_method", "llm_fallback_compound")
        }
