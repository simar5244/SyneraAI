...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Ensure Atlas Search index exists (no-op if already present)
    try {
      await db.command({
        createSearchIndexes: [
          {
      ...
      // ignore if already exists or command unsupported
    }

    // Generic 'how many <collection>' handler
    const howManyMatch = query.toLowerCase().match(/how many (\w+)/i);
    if (howManyMatch) {
      const name = howManyMatch[1];
      const collections = (await db.listCollections().toArray()).map(c => c.name);
      const matchCols = collections.filter(c => c.toLowerCase() === name || c.toLowerCase() === `${name}s`);
      if (matchCols.length) {
    ...
    // Dynamic structured query step
    // 1. Introspect schema
    const schema = {};
    for (const colName of (await db.listCollections().toArray()).map(c => c.name)) {
      const sample = await db.collection(colName).findOne();
      schema[colName] = sample ? Object.keys(sample) : [];
    }
    // 2. Ask LLM to construct aggregation
    let structResults = [];
    try {
      const structPrompt = `You are a MongoDB expert. Database schema: ${JSON.stringify(schema)}. Question: "${query}". Return only a JSON with keys 'collection' and 'pipeline' (array of stages).`;
      const structResp = await model.generate({prompt: structPrompt});
      const { collection: targetCol, pipeline } = JSON.parse(structResp.text);
      // Execute aggregation
      structResults = await db.collection(targetCol).aggregate(pipeline).toArray();
    } catch (e) {
      // ignore errors and fallback
      structResults = [];
    }
    if (structResults.length) {
      const hist = await getConversationHistory(historyCollection, session_id);
      const responseText = await generateNaturalLanguageResponseWithGemini(model, query, hist, structResults);
    ...
    // Fast direct MongoDB text search across all collections
    const allCols = await db.listCollections().toArray();
    let directResults = [];
    for (const { name: col } of allCols) {
      if (col === HISTORY_COLLECTION_NAME) continue;
      // auto-create Atlas Search index per collection
      try {
        await db.command({
          createSearchIndexes: [{ collection: col, name: ATLAS_SEARCH_INDEX, definition: { mappings: { dynamic: true } } }]
        });
      } catch {}
      let docs = [];
      try {
        ...
        // on missing text index, auto-create wildcard text index and retry
        if (err.message && err.message.includes('text index required')) {
          try {
            await db.collection(col).createIndex({ "$**": "text" });
            docs = await db.collection(col)
              .find({ $text: { $search: query } })
              .limit(5)
              .toArray();
      ...
      if (docs.length) {
        directResults.push(...docs.map(d => ({ ...d, __collection: col })));
      }
      if (directResults.length >= 10) break;
    }
    if (directResults.length) {
      const directResponse = await generateNaturalLanguageResponseWithGemini(model, query, history, directResults);
      await saveConversation(historyCollection, session_id, query, directResponse);
      return res.status(200).json({ response: directResponse });
    }

    // Fallback: sample-based QA across all collections
    const sampleLimit = 3;
    const sampleData = {};
    for (const col of (await db.listCollections().toArray()).map(c => c.name)) {
      if (col === HISTORY_COLLECTION_NAME) continue;
      try {
        const docs = await db.collection(col).find({}).limit(sampleLimit).toArray();
        sampleData[col] = docs;
      } catch {}
    }
    const qaPrompt = `You are a MongoDB assistant. Schema: ${JSON.stringify(schema)}. Sample data: ${JSON.stringify(sampleData)}. Question: "${query}". Provide a concise answer based on data.`;
    try {
      const qaResp = await model.generate({prompt: qaPrompt});
      const qaText = qaResp.text;
      await saveConversation(historyCollection, session_id, query, qaText);
      return res.status(200).json({ response: qaText });
    } catch {};

    // Final fallback: use Claude to reason over all data
    try {
      const finalPrompt = `You are a MongoDB expert and analyst. Database schema: ${JSON.stringify(schema)}. Sample documents: ${JSON.stringify(sampleData)}. Prior search hits: ${JSON.stringify(directResults)}. Conversation history: ${JSON.stringify(history)}. Question: "${query}". Provide a concise answer based strictly on the data.`;
      const claudeResp2 = await claudeClient.complete({
        prompt: finalPrompt,
        model: 'claude-2',
        max_tokens_to_sample: 500,
      });
      const answer2 = claudeResp2.completion;
      await saveConversation(historyCollection, session_id, query, answer2);
      return res.status(200).json({ response: answer2 });
    } catch {}
...
- Top-level fields: email, name, firstName, lastName, role, department, employee_department, hierarchy, user_status, company_name
