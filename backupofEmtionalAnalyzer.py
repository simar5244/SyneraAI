    ...
    try:
        ...
        if categories:
            ...
            for role in role_keys:
                ...
                if not role_categories or not categories or any(cat in categories for cat in role_categories):
...
def get_task_intensity(duty_description: str) -> Dict[str, Any]:
    ...
    if not is_compound:
        ...
        # If best match is below threshold, calculate weighted average of top matches
...
def calculate_collaboration_index(skillsFeedback: Dict[str, Any]) -> Dict[str, Any]:
    """
    Calculate collaboration index based on skills feedback data
    """
    if not skillsFeedback or not isinstance(skillsFeedback, dict):
        return {
    ...
    feedback_given = skillsFeedback.get("feedbackGiven", 0) or 0
    feedback_received = skillsFeedback.get("feedbackReceived", 0) or 0
    ...
    total_feedback = feedback_given + feedback_received
    ...
    # 60% based on activity, 40% based on quality
    collaboration_score = (0.6 * feedback_activity_score) + (0.4 * quality_score)
...
def analyze_employee_utilization(doc: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        collaboration_analysis = calculate_collaboration_index(skills_feedback)
    except Exception as e:
...
def perform_bulk_update(operations, collection_name=OUTPUT_COLLECTION):
...
def process_all_documents():
    ...
    cursor = db[OUTPUT_COLLECTION].find(
        {},  # No filter to get all documents
        {
            ...
            "skillsFeedback": 1,
            "responsibilities": 1  # Include this to potentially remove it
        }
    )
    
    merged_output_operations = []
    users_operations = []
    processed_count = 0
    ...
    for doc in cursor:
        try:
            ...
            if time_updates:
                merged_output_operations.append(
                    UpdateOne(
                        {"_id": doc_id},
                        {"$set": time_updates},
                        upsert=False
                ...
                # Also update users collection if a user with this email exists
            ...
            if job_duties:
                job_intensity_analysis = process_job_responsibilities(job_duties)
                # Add job intensity analysis to both collections
                merged_output_operations.append(
                    UpdateOne(
                        {"_id": doc_id},
            ...
            # Add utilization assessment to both collections
            merged_output_operations.append(
                UpdateOne(
                    {"_id": doc_id},
                    {"$set": {
            ...
            if len(merged_output_operations) >= 100:
                perform_bulk_update(merged_output_operations, OUTPUT_COLLECTION)
                merged_output_operations = []
...
def process_single_document(email: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # Perform bulk update for both collections
        if merged_output_operations:
            perform_bulk_update(merged_output_operations, OUTPUT_COLLECTION)
        if users_operations:
            perform_bulk_update(users_operations, USERS_COLLECTION)
        ...
        with db[OUTPUT_COLLECTION].watch(
            ...
            for change in stream:
                try:
                    ...
                    if email:
                        logging.info(f"Change detected for {email}, processing update...")
...
# --- Job Role Ontology ---
# This maps specific job titles to canonical forms for better matching
JOB_ROLE_ONTOLOGY = {
    # Customer Service / Call Center Roles
    "call center representative": "customer service representative",
    "call centre representative": "customer service representative",
    "call center agent": "customer service representative",
    "call centre agent": "customer service representative", 
    "call center operator": "customer service representative",
    "call centre operator": "customer service representative",
    "phone support": "customer service representative",
    "customer care representative": "customer service representative",
    "customer service agent": "customer service representative",
    "call center supervisor": "customer service manager",
    "call centre supervisor": "customer service manager",
    "customer service supervisor": "customer service manager",
    "call center team lead": "customer service manager",
    "call centre team lead": "customer service manager",
    ...
    "software assistant": "junior software developer",
    "junior programmer": "junior software developer",
    "senior programmer": "senior software developer",
    "senior coder": "senior software developer",
    "senior software engineer": "senior software developer",
    "lead developer": "technical lead",
    "lead engineer": "technical lead",
    "software architect": "technical lead",
    "software development manager": "development manager",
    ...
    # Management Roles by Domain
    "hr manager": "human resources manager",
    "human resources director": "senior human resources manager",
    "recruiting manager": "human resources manager",
    ...
    "budget manager": "financial manager",
    ...
    # Special case: Low-importance managers
    "snack manager": "facilities coordinator",
    "certificate manager": "administrative coordinator",
    "party manager": "event coordinator",
...
# --- Job Categories for Context-aware Matching ---
JOB_CATEGORIES = {
    "software_development": [
        "software", "developer", "engineer", "programmer", "coder", "application",
        "web", "mobile", "frontend", "backend", "fullstack", "architect", "coding"
    ],
    "data_science": [
        "data", "scientist", "analyst", "analytics", "machine learning", "ai", 
        "artificial intelligence", "statistics", "statistical", "big data"
    ],
    "design": [
        "designer", "ux", "ui", "user experience", "user interface", "graphic", 
        "visual", "creative", "art", "artist"
    ],
    "management": [
        "manager", "director", "head", "chief", "lead", "coordinator", "supervisor",
        "executive", "vp", "vice president", "c-level"
    ],
    "administrative": [
        "admin", "administrative", "receptionist", "secretary", "assistant", "clerk",
        "office", "support", "coordinator"
    ],
    "customer_service": [
        "customer", "service", "support", "help desk", "call center", "call centre",
        "care", "representative", "agent", "concierge"
    ],
    "sales": [
        "sales", "account", "business development", "relationship", "client", 
        "revenue", "deal", "territory"
    ],
    "marketing": [
        "marketing", "brand", "content", "social media", "seo", "ppc", "advertising",
        "campaign", "market research", "communications", "pr", "public relations"
    ],
    "finance": [
        "finance", "financial", "accounting", "accountant", "controller", "budget",
        "banking", "investment", "analyst", "treasury", "auditor"
    ],
    "hr": [
        "hr", "human resources", "recruiter", "recruiting", "talent", "training",
        "personnel", "benefits", "compensation", "learning", "development"
    ],
    "operations": [
        "operations", "logistics", "supply chain", "procurement", "purchasing",
        "inventory", "warehouse", "shipping", "receiving", "distribution"
    ],
    "it": [
        "it", "information technology", "network", "system", "infrastructure",
        "support", "help desk", "security", "database", "server", "cloud"
    ]
}

# --- Benchmark Embeddings Initialization ---
def initialize_vector_search():
    """Initialize vector search indexes for task and role matching"""
    ...
    # Initialize task vector index
    if benchmark_descriptions and benchmark_embeddings_cache is not None:
        try:
            # Get dimension of embeddings
            dimension = benchmark_embeddings_cache.shape[1]
            
            # Initialize FAISS index - using L2 distance (can be converted to cosine similarity)
            task_vector_index = faiss.IndexFlatL2(dimension)
            
            # Add benchmark task vectors to the index
    ...
    try:
        # Get all role keys and encode them
        role_keys = list(ROLE_COMPLEXITY.keys())
